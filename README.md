...
## 사용 흐름
1. 사이드바 `데이터 소스/연결`에서 실시간 캡처 방식 선택
2. `Chrome 확장 캡처` 모드: 확장 설치/활성화 후 `디버깅 모드 시작`
3. `Playwright 팝업` 모드(로컬 전용): `디버깅 모드 시작`
4. 테스트 사이트에서 실제 액션 수행
5. `실시간 테스트 화면`에서 즉시 반영 확인
6. `QA 리포트 화면`에서 API/세션 기준 판정 및 다운로드
